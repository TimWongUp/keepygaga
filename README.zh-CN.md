...
交互安装会先让用户确认或输入 Memory Root，再检测可用宿主并由用户选择。已有 `agents-memory` 记忆树可以直接接入；配置完成后，后续新增 Agent 会自动复用该目录。`--yes` 在 TTY 中也不询问输入，复用已配置的 Memory Root 或平台默认目录，并要求显式列出全部目标：
...
只更新当前宿主时，下载新版 wheel 与同一 Release 的 `SHA256SUMS`，在安装前立即重新校验将要执行的绝对路径，把该命令的 `UV_TOOL_DIR` 设为规划版 `status` 返回的 `lifecycle.tool_root`，再运行 `uv tool install --force /absolute/private/path/keepygaga-X.Y.Z-py3-none-any.whl`，最后执行 `keepygaga install --yes --host HOST`。所有宿主共用被替换的运行时，因此 `install` 会同时从其他已记录宿主中删除已退役动作的 Keepygaga Hook 命令（`sibling_hooks`）。它不新增任何 Hook，手动移除的 Hook 不会被写回；这些宿主的其他 Hook、MCP 注册和规则保持不变。整体对齐仍落后的宿主列为 `stale_hosts`；报告该列表，确定要对齐所有已记录宿主时再运行 `keepygaga repair --yes`。
