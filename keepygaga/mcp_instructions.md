...
- The canonical project index is the optional dynamic page `areas/projects.md`, which may be absent before its first Fact. Before creating it, list `areas` and rename an equivalent conforming legacy page. If none exists, use `create` with the initial project Fact. If legacy pages cannot converge through rename and exact updates without deleting history, stop and ask the user to lead the migration; do not create a duplicate canonical page. Keep exactly one Fact per maintained project, beginning with its stable project name and containing a concise brief, its canonical project Authority, and latest verified integrated outcome when available. For Git projects use a normalized credential-free canonical remote URL and render it as a Markdown autolink, for example `<https://github.com/owner/repo>`. Verify that remote's primary branch through a fresh fetch or live remote API; normally this is `origin/main`, but an arbitrary fork named `origin` is not automatically authoritative. A changed integrated capability, fix, performance improvement, architecture, or supported integration updates the outcome without a major-milestone threshold. Summarize the result, not a commit log. Repository-only formatting, tests, or editorial changes do not advance progress unless the project's brief or Authority changes. Record a published version only when a published Release on the canonical remote points to a tag commit containing the summarized outcome; otherwise mark it unreleased. A reserved source version, local branch, open PR, or installed host version is not publication evidence. When no durable remote or direct source exists, use a local path and a directly verified completed outcome. If current source verification is unavailable, retain the existing Fact rather than infer progress. Plans, blockers, next steps, test results, deployment instances, and runtime state stay in the project Authority or live direct source.
...
- After the initial project-index convergence above, a previously unlisted project on the existing canonical page is `new` and uses `add`. A changed brief, Authority, integrated outcome, or verified release status `refines` the existing project Fact and must use `update`. Replace the previous outcome in place; full progress history stays in the project Authority.
