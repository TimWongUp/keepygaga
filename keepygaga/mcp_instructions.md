...
- Classify durable instructions by intended scope, not content type. A prose-style or code-style instruction can be a cross-Agent user preference, a rule for one Agent host, or a project rule. Keep cross-Agent preferences only in `preferences.md`, one-host rules only in that host's effective global-rules entry, and project rules only in the project Authority. If the current user statement does not establish the scope, ask whether it is a user preference or a rule for a specific Agent before writing or moving it; ask about project scope when the context makes that plausible. Never retain the same meaning in more than one of these sources.
