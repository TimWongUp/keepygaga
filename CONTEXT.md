...
**Preference Scope**：
一条回应、工作、文风、代码风格或工具偏好的二选一归属：跨所有接入 Agent 的稳定用户偏好进入 `preferences.md`；仅约束某一 Agent 的规则进入该宿主的有效全局规则入口。当前用户陈述未明确范围时，写入或迁移前必须询问，不得根据内容类型猜测；已明确只适用于项目的规则在选择前排除并留在 Project Authority。
_Avoid_: 文风必定属于记忆、代码风格必定属于全局规则、把项目规则作为第三个迁移 owner、在多个来源重复同一含义

**Preference Fact**：
用户希望所有接入 Agent 跨任务长期遵循的回应、工作或条件检索偏好；遗漏它会使跨宿主行为持续偏离用户预期。
