...
**Preference Scope**：
一条回应、工作、文风、代码风格或工具偏好真正应适用的边界：跨所有接入 Agent 的稳定用户偏好进入 `preferences.md`；仅约束某一 Agent 的规则进入该宿主的有效全局规则入口；仅约束某一项目的规则进入 Project Authority。当前用户陈述未明确范围时，写入或迁移前必须询问，不得根据内容类型猜测。
_Avoid_: 文风必定属于记忆、代码风格必定属于全局规则、在多个来源重复同一含义
