...
For a current-host-only update, download the newer wheel and matching `SHA256SUMS`, rehash the exact absolute wheel path, set `UV_TOOL_DIR` to the `lifecycle.tool_root` returned by planned `status` for that command, run `uv tool install --force /absolute/private/path/keepygaga-X.Y.Z-py3-none-any.whl`, then run `keepygaga install --yes --host HOST`. All hosts share the replaced runtime, so `install` also removes Keepygaga Hook commands for retired actions from every other recorded host (`sibling_hooks`). It adds nothing, so Hooks you removed stay removed, and it leaves their other Hooks, MCP registrations, and rules unchanged. Hosts whose full reconciliation is still behind are listed as `stale_hosts`; report them, and run `keepygaga repair --yes` when you intend to reconcile every recorded host.
