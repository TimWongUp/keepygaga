...
For a current-host-only update, download the newer wheel and matching `SHA256SUMS`, rehash the exact absolute wheel path, set `UV_TOOL_DIR` to the `lifecycle.tool_root` returned by planned `status` for that command, run `uv tool install --force /absolute/private/path/keepygaga-X.Y.Z-py3-none-any.whl`, then run `keepygaga install --yes --host HOST`. All hosts share the replaced runtime, so any recorded host still wired for the previous release may call Hook commands that no longer exist; `install` lists them as `stale_hosts`. Report that list, and run `keepygaga repair --yes` when you intend to reconcile every recorded host.
