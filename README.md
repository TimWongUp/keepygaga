...
Keepygaga supports source-checkout installation only. GitHub Release packages
are not an installation channel. Only an unmodified checkout at the latest
commit on the upstream `main` branch is supported.
...
Update an unmodified checkout to the latest upstream `main`, record the exact
commit with `git rev-parse HEAD`, rerun `uv sync`, and rerun the same per-host setup command.
...
- Applied mutations return changed-page snapshots and an already-rendered server
  receipt. Reuse those snapshots for subsequent convergence, and echo the
  receipt exactly once; never invent, rewrite, or echo one for reads, no-ops,
  skips, or failures.
