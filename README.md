...
For a current-host-only update, download the newer wheel and matching `SHA256SUMS`, rehash the exact absolute wheel path, set `UV_TOOL_DIR` to the `lifecycle.tool_root` returned by planned `status` for that command, run `uv tool install --force /absolute/private/path/keepygaga-X.Y.Z-py3-none-any.whl`, then run `keepygaga install --yes --host HOST`. All hosts share the replaced runtime, so `install` also realigns the Keepygaga-owned Hooks of every other recorded host (`sibling_hooks`), removing commands the new runtime no longer accepts; their MCP registrations and rules are left unchanged. Hosts whose full reconciliation is still behind are listed as `stale_hosts`; report them, and run `keepygaga repair --yes` when you intend to reconcile every recorded host.
