...
`keepygaga install` projects only Keepygaga-owned commands into the selected host's native Hook configuration and removes Keepygaga Hook commands for retired actions from other recorded hosts, which share the same runtime; it adds no Hooks to them. Rerunning setup replaces current or legacy Keepygaga entries, including obsolete closeout commands, and preserves unrelated Hooks. `keepygaga uninstall` removes those owned entries only.
