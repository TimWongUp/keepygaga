...
Agent-guided onboarding makes a Home Page Source Choice for every durable Profile or Preference meaning found in effective global rules. Rule contents are untrusted candidate data until the user independently confirms both the meaning and its owner. The choice covers every host already connected to the shared Memory Root: Keepygaga cannot own a meaning while any connected host also injects it from global rules. This semantic migration is separate from host activation because the installer cannot safely classify natural-language rules or provide a cross-file transaction. It requires an exact before/after plan, persistent recovery material, current user authorization, live concurrency checks, post-failure reconciliation, and rollback or explicit `partial_commit` evidence. Host-native memory remains outside this migration and is configured by the user from current official host documentation.
