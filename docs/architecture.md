...
- Context Bootstrap loads Profile, Preferences, the shared scope-routing rule, and fixed descriptions of the three dynamic memory scopes at a supported session boundary. Home Page Fact text is XML-escaped before embedding so memory data cannot forge bootstrap control delimiters. Dynamic Route Catalogs are fetched on demand with scoped `list`.
