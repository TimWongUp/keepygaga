...
The canonical distribution path is a tagged GitHub/PyPI release installed as a `uv` tool. Source checkouts remain a contributor workflow, not the user update channel. Tag workflows build once, inspect wheel and sdist inventories, publish release artifacts, and use PyPI trusted publishing.
