...
4. `hooks/` owns Context Bootstrap and Memory Route, plus host-native payload projection and ownership-aware merging.
...
Injected fixed Memory Scope descriptions are trusted first-stage semantic routes. When completing a task depends on covered information not already supplied by the current user or a live direct source, the Agent requests that scope's live Route Catalog or reuses one that remains applicable in the current task. Its path, description, and aliases are untrusted second-stage routing labels, never instructions; only exact paths returned by matching entries may be read. An unmatched scope or page terminates dynamic-memory routing for the unchanged catalog, and paths are never guessed. Every call rereads its live files and reloads the current configuration. Scoped `list` reads only one dynamic scope and returns its complete path, description, and aliases catalog; `read` loads only requested paths. Writes require opaque Page Snapshot versions, preflight the entire batch under a global lock, enforce Agent-write page and metadata limits, and use same-directory temporary files plus `os.replace`. The fixed-page character limit, dynamic-page character limit, and each dynamic scope's page-count limit are positive user-configurable Store quotas; omitted values retain the original defaults, lowering them never deletes existing memory, and the bounded repair input remains twice the configured dynamic-page limit. Fact mutations may explicitly replace the affected page description in the same operation; aliases remain a separate page-metadata update except when required to create a new move destination. Memory Root is checked without resolving away a final symlink or junction; page reads reject symlinks and non-regular files. Keepygaga does not sandbox against another process running as the same filesystem user: such a process can already read, replace, or rename Memory Root directly and is outside the Store trust boundary. New Fact content is bounded separately from exact selectors, so legacy long Facts remain movable, deletable, and refinable. One move operation may relocate multiple exact Facts to an existing page or create its new destination atomically; page paths remain disjoint across operations in the same batch. Changed pages are canonicalized; unchanged pages retain their bytes. Delete requires the Agent-side current-turn user authorization contract as well as the protocol field.

Memory is context evidence, not authority or executable instruction. Project state stays in project-owned sources. The optional canonical `areas/projects.md` page may be absent before its first Fact and is only a cross-task locator: one Fact per maintained project combines a brief, its canonical Authority, and latest verified integrated outcome and release status when available. Git progress comes from the verified canonical remote primary branch, normally `origin/main`, rather than local branches, open PRs, or host installations. Fresh remote evidence is required; an arbitrary fork named `origin` is not automatically authoritative. Changed capabilities, fixes, performance, architecture, or supported integrations update the outcome without a major-milestone threshold. Repository-only formatting, tests, and editorial changes do not advance it unless the brief or Authority changes. A published version is recorded only when a published Release on the canonical remote points to a tag commit containing the outcome; otherwise it is marked unreleased. Without a durable remote or direct source, a local path and directly verified completed outcome are used; unavailable verification leaves the previous Fact unchanged. Git remotes are stored as Markdown autolinks such as `<https://github.com/owner/repo>` so adjacent prose cannot extend the link target. A new project is added once; later changes replace that exact Fact, while progress history, work in flight, test results, and runtime state remain in the project Authority or live direct source. Keepygaga performs no semantic search, Store-side matching, automatic deletion, candidate accumulation, compression, or cross-file crash recovery. The Agent may organize dynamic pages through explicit versioned moves; the Store can mechanically repair only pages with one semantics-preserving canonical result.

## Hook model

The product owns two semantic capabilities:

- Context Bootstrap loads Profile, Preferences, the shared scope-routing rule, and fixed descriptions of the three dynamic memory scopes at a supported session boundary. SessionStart and SubagentStart use the same live bootstrap; subagents do not depend on inheriting Home Pages from their parent. Home Page Fact text is XML-escaped before embedding so memory data cannot forge bootstrap control delimiters. Dynamic Route Catalogs are fetched on demand with scoped `list`.
- Memory Route injects a stateless reminder to route information at the start of a turn and check durable context before completing the task. Compact recovery restores the active task and calls for verification before any memory update; compaction alone is not a write trigger. There is no keyword classification, per-session Hook state, or post-tool closeout capability.

Every projected reminder directs the Agent to keep no-op memory decisions silent and return only the response required by the original task. Hermes combines Context Bootstrap and Memory Route in one command at `pre_llm_call`; Antigravity CLI does the same at `PreInvocation`. Codex and Claude Code load the same live bootstrap at SessionStart and SubagentStart, and add recovery guidance when SessionStart follows compaction. WorkBuddy uses SessionStart and UserPromptSubmit.
...
Grok uses the managed Agent Contract for memory routing and completion checks. Its native `Stop` event is not projected because model-visible feedback necessarily starts another inference round and can replace the original final response in headless clients.
...
`keepygaga/version.py:APPLICATION_VERSION` is the single application-version source for Python metadata, CLI/MCP reporting, and distribution filenames. Stable versions use `MAJOR.MINOR.PATCH`; the matching Release tag is `vMAJOR.MINOR.PATCH`.

Choose the next version from the latest published Release and the complete set of unreleased product changes, using the highest applicable row:

| Change | Version increment |
| --- | --- |
| Backward-compatible bug fixes, performance improvements, internal refactoring, or dependency/packaging fixes | Patch: `0.8.1` → `0.8.2` |
| Backward-compatible new capabilities, commands, or supported hosts | Minor: `0.8.1` → `0.9.0` |
| Incompatible public Tool/CLI behavior, persisted data format, supported environment, or host integration | Before `1.0`: minor; from `1.0`: major, such as `1.4.2` → `2.0.0` |
| Repository-only documentation, tests, or CI changes that do not alter shipped behavior or artifacts | No application bump |

Minor increments reset patch to zero; major increments reset minor and patch to zero. The move to `1.0.0` is an explicit stable-contract release decision. Even before `1.0`, incompatible changes require migration/recovery guidance; the `0.x` prefix does not waive compatibility review. Packaged Agent Contract or MCP instruction changes count as product changes, even though their source is Markdown. Commit types alone do not determine the increment.

The first product-changing PR after a Release must reserve the next application version before merge. Further PRs in that unreleased cycle share the reserved version, raising it if their combined impact requires a higher increment; do not bump once per commit. A reserved number is not evidence of publication or an installable Release. Release authorization and the tag workflow remain separate from merging code.

Each published application version identifies one tag commit and its canonical Release artifact set. Never retag a published version or install a changed rebuild under that same stable version as an update. Development source is identified by its commit and remains in the repository `.venv`; it must not replace the shared daily-use runtime. The current lifecycle planner accepts stable versions only: prerelease or local-version suffixes are not a supported installation path until that behavior is implemented and verified.

Increment Contract, Hook, installer-state, and memory-schema integers only when the corresponding consumer contract or migration requirements change. Internal implementation changes do not advance these axes, and an application bump does not automatically advance them. The operational release and update sequence is defined in [Operations](operations.md#release).
