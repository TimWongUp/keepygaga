...
Release tags are `v<application-version>`. The tag workflow reruns tests and static checks, builds wheel and sdist, verifies inventory, and creates a GitHub Release with a `SHA256SUMS` asset. The first successful build artifact in a workflow run is canonical for both GitHub and PyPI, including full and failed-job reruns. A failed run resumes its draft, uploads only missing assets, derives the checksum manifest from the stored distributions, and publishes only after the asset set is internally consistent; reruns never overwrite existing release assets. PyPI runs only after the GitHub Release succeeds and tolerates files already published by a partial attempt. It is an explicit secondary gate: a shell policy step enables it only when the repository variable `PYPI_PUBLISH_ENABLED` is exactly `true`, preserving case-sensitive fail-closed behavior. Before enabling that variable, protect the `v*` tag namespace, require approval on the `pypi` environment, and configure the PyPI trusted publisher; never add a publishing token to the repository. An unset or differently cased variable skips only the PyPI job and does not block the GitHub Release.
