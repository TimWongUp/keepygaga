...
To update through the current GitHub Release channel, download the newer wheel and matching `SHA256SUMS`, rehash the exact absolute wheel path, set `UV_TOOL_DIR` to the `lifecycle.tool_root` returned by planned `status` for that command, run `uv tool install --force /absolute/private/path/keepygaga-X.Y.Z-py3-none-any.whl`, then run `keepygaga install --yes --host HOST` to reconcile only the current host. Because every recorded host runs Hooks through the replaced runtime, `install` also removes only Keepygaga Hook commands for retired actions from the other recorded hosts and reports each result under `sibling_hooks`. It never adds Hooks, so live configuration that removed Keepygaga Hooks stays authoritative. A sibling failure is reported without failing the current host; a partial Grok write keeps the applied file and backup under `components`. Their MCP registrations, rules, and reconciliation records are unchanged, so hosts whose record differs from the running release are listed as `stale_hosts`; report them and run `keepygaga repair --yes` when every recorded host should be reconciled. `keepygaga upgrade`, with or without `--yes`, returns `manual_review` and a nonzero exit code without changing runtime, state, or hosts. It provides explicit wheel replacement guidance instead of using `uv tool upgrade` against the original wheel. Local archives lose the original download URL in PEP 610 metadata, so planned status requires origin verification before runtime replacement, while same-version initialization and host reconciliation remain available; package-manager ownership alone is insufficient. `pipx`, unknown, and state-mismatched owners fail closed for manual owner-correct replacement instead of being routed through `uv`.
...
- After explicit wheel replacement, verify the selected version through a fresh installed launcher before reconciling the intended hosts. If reconciliation fails, retain the runtime/version evidence and repair the reported host; runtime replacement alone does not prove host activation.
