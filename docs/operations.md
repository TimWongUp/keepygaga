...
`host setup` 对有效记忆树的 soft warning 只接受 `split_recommended` 与 `dynamic_page_limit_exceeded`；POSIX 过宽权限 warning 会阻断 setup，要求先收紧权限或接受该树当前不可接入新宿主。动态页超限不阻断 MCP、Agent Contract 或宿主配置升级，因为 Store 已阻止继续创建页面。
...
- `capacity_exceeded`：动态页已达上限，读取、更新、移动、重命名和删除仍可用；删除页面后再创建。Doctor 对已超额的树给出 warning，不自动删页；`host setup` 把它当作 soft warning 继续。
...
- `host setup codex` 或 `host uninstall codex` 的 `partial_commit`：响应中的 component 已应用部分必须按其 `backup` / `recovery` 现场处理；setup 时 MCP 未成功 apply 则 rules 不会写入，uninstall 时 Hook 已拆除而后续 component 失败则按返回现场处理，不回写 Memory Root。
- 任一非 Codex `host setup` 或 `host uninstall` 的 `partial_commit`：以返回的 `components`、`path` 与 `backup` 为现场，不假设不同宿主配置能够统一回滚；核对后幂等重跑同一个宿主命令。Hermes 首次写入 AHR Hook 后若 `approval_required=true`，由用户明确选择的 runtime 仍需通过 Hermes 自身 Hook allowlist，再用 `hermes hooks doctor` 验证。
