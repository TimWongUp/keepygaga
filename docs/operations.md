...
To update through the current GitHub Release channel, download the newer wheel and matching `SHA256SUMS`, rehash the exact absolute wheel path, set `UV_TOOL_DIR` to the `lifecycle.tool_root` returned by planned `status` for that command, run `uv tool install --force /absolute/private/path/keepygaga-X.Y.Z-py3-none-any.whl`, then run `keepygaga install --yes --host HOST` to reconcile only the current host. Because every recorded host runs Hooks through the replaced runtime, `install` also realigns only the Keepygaga-owned Hooks of the other recorded hosts and reports each result under `sibling_hooks`; a sibling failure is reported without failing the current host. Their MCP registrations, rules, and reconciliation records are unchanged, so hosts whose record differs from the running release are listed as `stale_hosts`; report them and run `keepygaga repair --yes` when every recorded host should be reconciled. `keepygaga upgrade`, with or without `--yes`, returns `manual_review` and a nonzero exit code without changing runtime, state, or hosts. It provides explicit wheel replacement guidance instead of using `uv tool upgrade` against the original wheel. Local archives lose the original download URL in PEP 610 metadata, so planned status requires origin verification before runtime replacement, while same-version initialization and host reconciliation remain available; package-manager ownership alone is insufficient. `pipx`, unknown, and state-mismatched owners fail closed for manual owner-correct replacement instead of being routed through `uv`.
