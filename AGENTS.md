...
- 安装、升级或修复宿主接线时先按用户请求确定目标 Agent；默认只处理当前工作的 Agent，只有用户明确要求时才加入其他目标。当前确定性命令为 `keepygaga host setup codex|claude-code|workbuddy|grok|hermes|antigravity`，每次只处理一个宿主；`antigravity` 指 Antigravity CLI，不等同于 Gemini CLI。Hook 接线先读 `docs/hooks/README.md`，再只读每个目标 Agent 的对应专页；没有兼容 runtime 时仍完成该目标的 MCP 与 Agent Contract 安装并明确报告 Hook 跳过，不生成临时 Hook 实现，也不修改范围外 Agent 的全局规则或 Hook。
