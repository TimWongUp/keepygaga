...
- Memory Scope 的固定 description 是一级语义路由条件；任务所需且当前输入或直接真源未提供的信息落入其覆盖范围时，先获取该 scope 的 live Route Catalog，再按返回的 path、description 与 aliases 选择页面。无匹配项时不猜路径或读取动态记忆。
