...
- `profile.md` 只保存三个月后仍应成立的身份与背景；可选动态页 `areas/projects.md` 是持续项目的规范索引，首条项目 Fact 写入前可以不存在，每个项目只保留一条以稳定项目名开头、包含简短简介、项目真源及可选最新重大节点的 Fact。首次写入前先列出 `areas`：有等价且已符合单 Fact 模型的旧页面时重命名复用，没有时用 `create` 连同首条项目 Fact 创建；存在不能通过重命名和精确更新收敛的冲突旧页面时由用户主导迁移，不另建重复索引。Git 项目优先以 Markdown 自动链接 `<https://github.com/owner/repo>` 记录不带凭据的规范远端 URL；没有持久远端或直接真源时才记录本机路径。后续新增项目用 `add`，项目简介、真源或最新重大节点变化时用 `update` 替换原 Fact；完整历史、详情、决策、计划、阻塞、下一步、普通提交、单次任务、测试结果和当前运行状态留在项目 Authority。`preferences.md` 只保存用户希望所有接入 Agent 跨任务长期遵循的回应、工作与条件检索偏好；只约束某一宿主的规则留在该宿主全局规则，项目规则留在项目 Authority。文风与代码风格不决定归属；当前用户未说明适用范围时，写入或迁移前先询问是跨 Agent 用户偏好还是特定 Agent 规则，同一含义不得写入两处。所有页面使用统一 basis 与容量合同，具体写入边界以当前 Tool schema 和 Store 错误为准。
